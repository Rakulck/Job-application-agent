...
```
job-agent/
├── CLAUDE.md                      ← Project overview, current phase, pointers
...
7. Update Supabase applications table with result:
   - For failed/skipped jobs: populate manual_apply_url with job.job_url
   - This enables "Apply Manually" link in frontend for user fallback
...
2. JobTable (main view)
   - Columns: Company | Title | Location | Role | Status | Applied At | Action
   - Status shown as coloured badge (applied=green, failed=red, skipped=gray, captcha=yellow)
   - Action column shows:
     * "View Resume" link for applied jobs (modal to preview tailored resume + download PDF)
     * "Apply Manually" link for failed/skipped jobs (opens LinkedIn job URL in new tab)
     * "Retry" option for captcha_blocked jobs (re-run application)
...
Manual Apply Implementation:
- When status = 'failed' or 'skipped', populate manual_apply_url from jobs.job_url
- Show as blue clickable link: <a href={job.job_url} target="_blank">Apply Manually</a>
- This allows users to quickly apply to jobs that couldn't be auto-filled
```
